...
Use the following names for these summary variables: `average_pH` and `std.dev_pH`.
...
3. We want to re-order the Treatment groups on the $x$-axis so that `"Control"` appears first, followed by `"wildtype"` and then `"budB"`.
...
To change the order of the `Treatment` groups on the $x$-axis, we actually have to modify the `Treatment` variable and re-assign an order for its levels (`"Control"`, `"wildtype"`, `"budB"`).
...
#### Exercise!
Remembering that we want our labels to read `"Control"`, `"wildtype"`, then `"budB"`, complete the code below to re-order the levels of the `Treatment` variable. 
