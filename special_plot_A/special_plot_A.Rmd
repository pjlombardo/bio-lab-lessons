...
1. Since we are dealing with `pH`, which was only measured at the end of the experiment, we need to `filter()` to only keep rows where `TimeGroup` equals "100hr".
2. Because we want to display the average (mean) pH for each group, we need to take the data and `group_by()` the `Treatment` variable then `summarize()` the data for each Treatment group to calculate the mean `pH`.
3. We will also need to add columns (using `mutate()`) for where the error bars will start and stop: `mean` $\pm$ `2*std.dev`.
...
1. We want better axis labels and a title; specifically, we want 
    ...
    * a title of "Leaf pH at 100 hours by Treatment."
2. We want the $y$-axis to have markings for every unit of pH (i.e., it should be labeled 0, 1, 2, ...7).
3. We want to re-order the Treatment groups on the $x$-axis to ... ***Prescription here***
...
ggplot(data = plot_df,
    ...
    geom_col(aes( fill=Treatment ))+
    geom_errorbar(aes(ymin=low, ymax =high), width = 0.1)+
    labs(y="Average pH", title = "Leaf pH at 100 hours by Treatment")
```
...
### Add something how to re-order the levels of the $x$-axis
using `fct_relevel()`, or using `arrange()`.
