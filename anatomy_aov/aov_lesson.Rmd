...
(*Note:* provide a link to a google sheet of the data, and then have them look in R.)

(*Note*: Show code for loading a spreadsheet (probably excel downloaded from google sheets, but give all options.))
...
* *Note:* Maybe keep `Time` variable so they can verify that they have the right data.
...
> At the population level (so extending beyond just the data we collected), *at least one* treatment group will have a different mean lesion length after 72 hours than the others.
