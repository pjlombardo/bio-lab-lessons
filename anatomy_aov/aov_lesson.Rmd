...
<a href="https://docs.google.com/spreadsheets/d/14SjsEqxfG1aucubCJkq41gMLh47XJ5b2R1J0PMAkar4/edit?usp=sharing" target="_blank">Google Sheet of Experiment Data</a>
...
experiment
```
*Note:* The data is in "tidy" format already. This means that each row corresponds to a measurement taken on a given leave at a particular time point (20hr, 50hr, or 100hr).  Each endpoint metric, as well as a time and treatment labels, are stored in their own columns.^[Also note that `pH` has `NA` values whenever `TimeGroup` is not equal to 100 hours; this is because we do not measure pH until the end of the experiment.]

(*Note*:  Something on `NA` either as empty cell in excel or NA in csv. Make a note that you don't need to type NA in the excel or csv, just leave a blank cell.)
...
(*Note:* Drop the language of "association" and just review indpendent/dependent variables.)
...
(*Note:* Add parentheticals to use explanatory/response variable in the definitions below.)
...
```{r response-question}
question("In the P. carotovorum experiment, what variable or variables from the data frame do we think of as measuring a *response* to changes administered by the researcher? <br> *Select all that apply*.",
...
Recall that we are only analyzing data from the *end of the experiment.* This means we want rows where `TimeGroup` equals (`==`) the "100hr" mark.  Use the code chunk below to do the following:
...
* *Note*: put in a reminder that the comparison is case sensitive, and needs to perfectly match what's in the data frame.  Also, why do we need quotes for some, but not others (measurements/levels need quotes, but variables do not.)
* *Note:* Maybe keep `TimeGroup` variable so they can verify that they have the right data.

Sift through the output to verify that you have the data you need to continue.
...
* Difference in means comparing `Wild` and `Control`
* Difference in means comparing `Wild` and `budB`
...
* We can use the confidence intervals to determine which group mean is larger.  
    * For example, if we compare `Wild` minus `Control`, and the confidence interval for the difference in means is entirely positive, it suggests the group mean lesion length for `Wild` type will be bigger than the group mean for `Control`.
...
The last column consists of our adjusted $P$-values. The `Wild-budB` comparison, for example, has an adjusted $P$-value of 0.058.
...
question("Between which two groups did Tukey's HSD detect a significant difference in means? Use a significance level (i.e. $P$-value threshold) of 0.05. <br> *Select all that apply*.",
    answer("Wild-budB", message = "This has an adjusted $P$-value of 0.058, which is not below 0.05. However, we do admit that this is *close*, and probably should raise an eyebrow on the researcher."),
    answer("Wild-Control", correct = TRUE),
    answer("Control-budB", message = "This has an adjusted $P$-value of 0.47, which is not below 0.05."),
    answer("Wild-Control-budB", message = "This isn't a comparison that Tukey's test does."),
    allow_retry = T,
    random_answer_order = T,
...
<center><img src="images/tukey2.png" alt="Tukey Output, P-values" width="75%"></center>
...
question("Considering the Tukey HSD output, which comparisons have a confidence interval that *contains* zero? <br> *Select all that apply*",
    answer("Wild-budB",correct=TRUE),
    answer("Wild-Control", message = "The lower limit of this confidence interval, `Wild-Control`, is about 4, which avoids zero."),
    answer("Control-budB", correct = TRUE),
    answer("Control-Wild-budB", message = "`Control-Wild-budB` isn't a comparison that Tukey's test does."),
    allow_retry = T,
    random_answer_order = T,
    post_message = random_praise()
  )
...
* We have evidence that at least one treatment group has a different mean lesion length than the other groups. Specifically,
    * Further, a post-hoc Tukey HSD finds sufficient evidence of a difference in lesion length group means between `Control` leaves and `Wild`-inoculated leaves.
...
We can go one step further. Notice that our confidence interval for `Wild-Control` is 4.9 mm to 68.8 mm. This is estimating the difference in means of `Wild` minus `Control`, and it is entirely positive! This suggests that the `Wild`-inoculated leaves have a larger mean lesion length than the control.  This is indicative of more disease severity.
...
Based on this, a researcher might expect that the leaves inoculated with `Wild`-type P. carotovorum would attempt to control the pH on the leaf.  Rather than focusing only on the `Wild` group, let's consider all three treatment groups.
...
quiz(
  ...
  }), message="*An ANOVA indicated that we have evidence at least one treatment group (`Wild`,`budB`,`Control`) had a different mean pH when compared to the others.  The results of a post-hoc test, Tukey's HSD, gave evidence that `Wild` had a different mean pH than both `Control` and `budB`. We did not find evidence for a difference in mean pH when comparing `budB` and `Control`.*"
)
)
```
...
quiz(
   question("Considering the comparison between `Control` and `budB`, what does the Tukey confidence interval for difference in means suggest?",
    ...
    answer("We do not have enough evidence for a difference between the means of these groups.", correct = TRUE),
    allow_retry = T,
    random_answer_order = T,
    post_message = random_praise()
  ),
  
  question("Considering the comparison between `Wild` and `budB`, what does the Tukey confidence interval for difference in means suggest?",
    answer("The mean pH for `Wild` is bigger than `budB`.",
           correct=TRUE),
    answer("The mean pH for `budB` is bigger than `Wild`.",message="The order is Wild minus budB. Is the confidence interval entirely *negative*? That's what we would require for this sentence to be true."),
    answer("We do not have enough evidence for a difference between the means of these groups.", message="Does the confidence interval contain zero? That's what we would require for this sentence to be true."),
    allow_retry = T,
    random_answer_order = T,
    post_message = random_praise()
  ),
  
  question("Considering the comparison between `Wild` and `Control`, what does the Tukey confidence interval for difference in means suggest?",
    answer("The mean pH for `Wild` is bigger than `Control`.",
           correct=TRUE),
    answer("The mean pH for `Control` is bigger than `Wild`.",message="The order is Wild minus Control. Is the confidence interval entirely *negative*? That's what we would require for this sentence to be true."),
    answer("We do not have enough evidence for a difference between the means of these groups.", message="Does the confidence interval contain zero? That's what we would require for this sentence to be true."),
    allow_retry = T,
    random_answer_order = T,
    post_message = random_praise()
