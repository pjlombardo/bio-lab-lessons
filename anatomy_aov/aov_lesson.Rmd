...
By implying that reproducing the experiment would not change the results, we are suggesting the difference we are detecting is a feature of the *populations* from which we draw, and not particular samples. Since each reproduction of the experiment would draw from the same populations, we would not expect the results to vary.

## Tukey's Honest Significant Difference Test
The ANOVA results ($P$-value = 0.0136 < 0.05) give us reason to believe that *at least one* population group mean is different from the others.  Running Tukey's Test will help us answer more interesting questions.
...
The most important information appears in the table output.  Each row corresponds to a pairing of treatment groups, and the difference of means test was computed using that order.  For example, the row labeled `Control-budB` gives information on the difference of means test where we look at `Control` minus `budB`.

The last column consists of our adjusted $P$-values.  Any row with an adjusted $P$-value indicates a test that provides evidence for a difference in means *between these two groups*.

<center><img src="images/tukey1.png" alt="Tukey Output, P-values" width="75%"></center>

The `Wild-budB` comparison, for example, has an adjusted $P$-value of 0.058. 
...
* This interval contains zero, so it remains possible that the difference in the populations means is zero (i.e., no difference). This aligns with the $P$-value above 0.05.
...
* At least one treatment group has a different mean lesion length than the other groups. Specifically,
    * A post-hoc Tukey HSD finds sufficient evidence of a difference in group means between `Control` leaves and `Wild`-inoculated leaves.
...
We could add more detail to this conclusion, but notice it is more informative than saying "*at least one group mean is different*".
...
Based on this, a researcher might expect that the leaves inoculated with `Wild`-type P. carotovorum would attempt to control the pH on the leaf.  Rather than focusin only on the `Wild` group, let's consider all three treatment groups.
...
3. *If this $P$-value is less than 0.05*, run a Tukey HSD to explore pair-wise differences.

We can use `final_df` for our data set, since we included `pH` in the variables.
```{r ph-anova, exercise = TRUE, exercise.lines=6}
# Code for 1
...
### Visualizing the differences in means from Tukey
