...
1. Because our $y$-axis aesthetic will present averages, we will need find the mean lesion length for each of our treatment groups.  Hence, we will `group_by()` the `Treatment` variable, and then `summarize()` to compute the mean `Lesion.mm`.
...
3. We will also need to add columns (using `mutate()`) for where the error bars will start and stop: `mean` $\pm$ `2*std.dev`. Since we'll be using `geom_errorbar()`, we also need to add columns (using `mutate()`) for where the error bars will start and stop: mean $\pm$ 2*std.dev. 
...
***NOTE WHEN WE USE A QUANTITATIVE VARIABLE, THIS WORKS FINE.  WE NEED THE GROUP AESTHETIC WHEN `x` IS A CATEGORICAL VARIABLE***
