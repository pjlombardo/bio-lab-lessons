...
ggplot(data = plot_df,
       aes(x = Time,
           ...
           color = Treatment,
           group = Treatment))+
    geom_point(position=position_dodge(width=.095)) + geom_line(position=position_dodge(width=.095))+
    geom_errorbar(aes(ymin=low, ymax =high), width = 0.1,
                  alpha = .5, position=position_dodge())+
    theme_classic()+
    labs(y="Average Lesion Length (mm)",
...
ggplot(data = .,
       aes(x = Time,
           ...
           fill=Treatment))+
    # geom_violin()+
    geom_boxplot(color='gray1',alpha=.3)+
    ...
    theme_bw()+
    labs(y="Lesion Length (mm)",
         title = "Separate boxplots by Treatment, with time progression\ninside each subplot, data overlayed")
```
...
ggplot(data = plot_df,      # link to the data frame plot_df
       # We want Time on the x axis, average lesion length on the y.
       aes(x = Time,
           y = average.Lesion.mm,
