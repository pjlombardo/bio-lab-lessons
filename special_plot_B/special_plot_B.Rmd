...
At 24, 48, and 72 hours, disease progression was monitored as the length of the soft rot area in millimeters (`Lesion.mm`).
...
2. Because our $x$-axis aesthetic involves multiple time points, we will actually need the summaries described in (1) above *for each time point*: 24, 48, and 72 hours.  We can accomplish this by adding `Time` to the `group_by()` function.
...
* `Time`: indicating the time point for that row
...
We can begin creating the summary data we need to for our plot.  Since we want to compute summaries for each `Treatment` group at each `Time`, we need to

1. Take our data frame and `group_by()` the `Treatment` and `Time` variables, then
...
1. We want better axis labels and a title; specifically, we want the $y$-axis to be "Average pH" and a title of "Leaf pH at 72 hours by Treatment."
2. We want the $y$-axis to have markings for every unit of pH (i.e., it should be labeled 0, 1, 2, ...7).

The first modification is straightforward, and simply involves adding `labs()` to the plot with arguments for `y` and `title`.  The second, involes *scaling* one of our aesthetics, namely `y`, which we can accomplish with `scale_y_continuous()`, since pH is measured on a continuous scale.
...
You have created a publication worthy plot of the experiment data.  The next section is a conclusion where we wrap up some of our ideas and present all the code we generated for this plot. 
